...
for (let i = 17; i <= 41; i++) {
    ...
    if (i === 26) id = '26b';
    DEFAULT_DATA.push({ id, name:`غير مسجل ${i}`, type:'نعجة', gender:'أنثى', birth:'', status:'حية', notes:'بيانات غير متوفرة', color:'', breed:'', weight:0, price:0, salePrice:0, financeNotes:'', kids:[], vaccines:[] });
}
DEFAULT_DATA.push({ id:'26c', name:'مكرر 26', type:'نعجة', gender:'أنثى', birth:'', status:'حية', notes:'مكرر من الجدول', color:'', breed:'', weight:0, price:0, salePrice:0, financeNotes:'', kids:[], vaccines:[] });
...
function loadData() {
    if (isFirebaseReady && collectionRef) {
        ...
        unsubscribeSnapshot = collectionRef.onSnapshot((snapshot) => {
            if (snapshot.empty) {
                DEFAULT_DATA.forEach(item => collectionRef.add(item).catch(console.error));
                sheepData = DEFAULT_DATA.map(normalizeSheep);
            } else {
                sheepData = [];
...
function loadFromLocalStorage() {
    ...
    if (stored) { try { sheepData = JSON.parse(stored).map(normalizeSheep); } catch(e) { sheepData = DEFAULT_DATA.map(normalizeSheep); } }
    else sheepData = DEFAULT_DATA.map(normalizeSheep);
    saveToLocalStorage();
    afterDataLoad();
}
...
function saveToLocalStorage() { localStorage.setItem('sheepFarmData', JSON.stringify(sheepData)); }
...
function deleteSheep(id) {
    ...
    deleteSheepFromFirebase(id).then(() => {
        if (!isFirebaseReady) { sheepData = sheepData.filter(s => s.id!==id); saveToLocalStorage(); afterDataLoad(); }
        closeModal();
...
function saveKid(e) {
    ...
    saveSheepToFirebase(s).then(() => {
        if (!isFirebaseReady) { saveToLocalStorage(); afterDataLoad(); }
...
console.log('نظام الديوان - النسخة المتطورة v2.0');